...
used_space = all_nodes[0].size  # first node is always the root
unused_space = 70_000_000 - used_space
target = max(0, 30_000_000 - unused_space)
print(f'{target = }')

best_fit = all_nodes[0]
for node in all_nodes[1:]:
    if node.size >= target:
        if node.size < best_fit.size:
            best_fit = node
print(f'{best_fit.name} {best_fit.size}')
